...
client.on("message", async message => {
        ...
        /* Commands */
            ...
            } else if (message.isMessageComponent() && message.customID.startsWith("ttt")) {
                await updateGrid(message);
            }
        }); async function updateGrid(message) {
        ...
        const xs_turn = xs <= os;
        const i = parseInt(message.customID[3]),
            j = parseInt(message.customID[4]);

        const buttonPressed = message.components[i - 1].components[j - 1];

        if (buttonPressed.label !== '_')
            return await message.reply("Someone already played there!", {
                ephemeral: true
            });
