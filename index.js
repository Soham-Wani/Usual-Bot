...
client.on("guildMemberAdd", async member => {
    member.guild.channels.cache.find(channel => channel.name.includes('welcome')).send({
            embeds: [new Discord.RichEmbed().setColor('#00ff00').setTitle(`Welcome __` + member.user.tag + `__`).setDescription(`Hey ${member}! Hope you enjoy!`).setImage(member.user.avatarURL)]});
});
