...
client.on("message", async message => {
    ...
    } else if (message.content.toLowerCase().startsWith(`${prefix}kick`) && !message.mentions.members.first() && message.content !== `${prefix}kick` && message.member.permissions.has("ADMINISTRATOR")) {
        message.channel.send(`Please include a valid user Type __${prefix}kick__ to know more.`)
